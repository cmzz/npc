...
零运行时依赖（纯标准库），`uv tool install` 即用。被 **[agent-spine](https://github.com/winewei/agent-spine)**（人驾驭 harness skill）与 **aidevos**（自治系统）作为子模块复用——同一套确定性工具，赋能不同上层的 LLM 执行，减少 LLM 处理确定性任务。
